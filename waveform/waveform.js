...
(function () {
  ...
  const COLOR_UNSOUND = '#333333';
  const COLOR_SOUND = '#ff6600';
  ...
  class Waveform {
    ...
    render() {
      ...
      this.lastPlaytime = 0;
      this._updateSegment(0, this.canvas.width, COLOR_UNSOUND);
    }
    ...
    update() {
      ...
      this._updateSegment(this._timeToPixels(start), this._timeToPixels(end), COLOR_SOUND);
    }
    ...
    _bindTimeUpdateEvent() {
          this.sound.on(TimeUpdateEvent, this.update, this);
    }

    _unbindTimeUpdateEvent() {
          this.sound.off(TimeUpdateEvent);
    }
  };
